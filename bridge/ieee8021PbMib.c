...
static int8_t
ieee8021PbEdgePortTable_Pep_BTreeNodeCmp (
	xBTree_Node_t *pNode1, xBTree_Node_t *pNode2, xBTree_t *pBTree)
{
	register ieee8021PbEdgePortEntry_t *pEntry1 = xBTree_entry (pNode1, ieee8021PbEdgePortEntry_t, oBTreeNode);
	register ieee8021PbEdgePortEntry_t *pEntry2 = xBTree_entry (pNode2, ieee8021PbEdgePortEntry_t, oBTreeNode);
	...
	return
		(pEntry1->u32CComponentId < pEntry2->u32CComponentId) ||
		(pEntry1->u32CComponentId == pEntry2->u32CComponentId && pEntry1->u32PepPort < pEntry2->u32PepPort) ? -1:
		(pEntry1->u32CComponentId == pEntry2->u32CComponentId && pEntry1->u32PepPort == pEntry2->u32PepPort) ? 0: 1;
}
...
ieee8021PbEdgePortEntry_t *
ieee8021PbEdgePortTable_Pep_getByIndex (
	uint32_t u32BridgeBasePortComponentId,
	uint32_t u32BridgeBasePort)
{
	register ieee8021PbEdgePortEntry_t *poTmpEntry = NULL;
	...
	poTmpEntry->u32CComponentId = u32BridgeBasePortComponentId;
	poTmpEntry->u32PepPort = u32BridgeBasePort;
	if ((poNode = xBTree_nodeFind (&poTmpEntry->oBTreeNode, &oIeee8021PbEdgePortTable_BTree)) == NULL)
	{
		xBuffer_free (poTmpEntry);
		return NULL;
	...
	ieee8021PbEdgePortEntry_t *poEntry)
{
	...
	xBTree_nodeAdd (&poEntry->oPep_BTreeNode, &oIeee8021PbEdgePortTable_Pep_BTree);
	...
	ieee8021PbEdgePortEntry_t *poEntry)
{
	...
	xBTree_nodeRemove (&poEntry->oPep_BTreeNode, &oIeee8021PbEdgePortTable_Pep_BTree);
	...
	ieee8021PbEdgePortEntry_t *poEntry, uint8_t u8RowStatus)
{
	...
	if (poEntry->u8RowStatus == u8RealStatus)
	{
		goto ieee8021PbEdgePortRowStatus_handler_success;
	}
	if (u8RowStatus & xRowStatus_fromParent_c &&
		((u8RealStatus == xRowStatus_active_c && poEntry->u8RowStatus != xRowStatus_notReady_c) ||
		 (u8RealStatus == xRowStatus_notInService_c && poEntry->u8RowStatus != xRowStatus_active_c)))
...
bool
ieee8021PbCnpRowStatus_handler (
	ieee8021PbCnpEntry_t *poEntry, uint8_t u8RowStatus)
{
	...
	{
	case xRowStatus_active_c:
		if (!((poEntry->u32CComponentId == 0) ^ (poEntry->u32SVid == 0)))
		{
			goto ieee8021PbCnpRowStatus_handler_cleanup;
		}
...
ieee8021PbCnpRowStatus_handler_success:
	
	return true;
	
	
ieee8021PbCnpRowStatus_handler_cleanup:
	...
	ieee8021PbPnpEntry_t *poEntry, uint8_t u8RowStatus)
{
	...
	case xRowStatus_active_c:
		...
		if (!ieee8021PbPnpRowStatus_update (poIeee8021BridgeBaseEntry, poEntry, u8RealStatus))
		{
			goto ieee8021PbPnpRowStatus_handler_cleanup;
		}
	...
	ieee8021PbCepEntry_t *poEntry)
{
	...
	if ((poSComponent = ieee8021BridgeBaseTable_getByIndex (poEntry->u32BridgeBasePortComponentId)) == NULL ||
		(poSComponent->u8RowStatus == xRowStatus_active_c && poSComponent->i32ComponentType != ieee8021BridgeBaseComponentType_sVlanComponent_c))
	{
	...
	if ((poCComponent = ieee8021BridgeBaseTable_createExt (ieee8021BridgeBaseComponent_zero_c)) == NULL)
	{
		goto ieee8021PbCepTable_createHier_cleanup;
	}
	poCComponent->i32ComponentType = ieee8021BridgeBaseComponentType_cVlanComponent_c;
	
	poEntry->u32CComponentId = poCComponent->u32ComponentId;
	
	if (!ieee8021BridgeBasePortTable_allocateIndex (poSComponent, &poEntry->u32BridgeBasePort))
	{
		goto ieee8021PbCepTable_createHier_cleanup;
	}
	...
	if ((poIeee8021BridgeBasePortEntry = ieee8021BridgeBasePortTable_getByIndex (poEntry->u32CComponentId, poEntry->u32BridgeBasePort)) == NULL &&
		(poIeee8021BridgeBasePortEntry = ieee8021BridgeBasePortTable_createExt (poCComponent, poEntry->u32BridgeBasePort)) == NULL)
	{
		goto ieee8021PbCepTable_createHier_cleanup;
	}
