...
static int8_t
mplsTunnelPerfTable_BTreeNodeCmp (
	xBTree_Node_t *pNode1, xBTree_Node_t *pNode2, xBTree_t *pBTree)
{
	register mplsTunnelPerfEntry_t *pEntry1 = xBTree_entry (pNode1, mplsTunnelPerfEntry_t, oBTreeNode);
	register mplsTunnelPerfEntry_t *pEntry2 = xBTree_entry (pNode2, mplsTunnelPerfEntry_t, oBTreeNode);
...
xBTree_t oMplsTunnelPerfTable_BTree = xBTree_initInline (&mplsTunnelPerfTable_BTreeNodeCmp);
	...
	uint32_t u32EgressLSRId)
{
	...
	poEntry->u32IngressLSRId = u32IngressLSRId;
	poEntry->u32EgressLSRId = u32EgressLSRId;
	if (xBTree_nodeFind (&poEntry->oBTreeNode, &oMplsTunnelPerfTable_BTree) != NULL)
	{
		xBuffer_free (poEntry);
		return NULL;
	}
	
	xBTree_nodeAdd (&poEntry->oBTreeNode, &oMplsTunnelPerfTable_BTree);
	return poEntry;
}
...
mplsTunnelPerfTable_getByIndex (
	...
	uint32_t u32EgressLSRId)
{
	register mplsTunnelPerfEntry_t *poTmpEntry = NULL;
	register xBTree_Node_t *poNode = NULL;
	...
	poTmpEntry->u32IngressLSRId = u32IngressLSRId;
	poTmpEntry->u32EgressLSRId = u32EgressLSRId;
	if ((poNode = xBTree_nodeFind (&poTmpEntry->oBTreeNode, &oMplsTunnelPerfTable_BTree)) == NULL)
	{
		xBuffer_free (poTmpEntry);
		return NULL;
	}
	...
	xBuffer_free (poTmpEntry);
	return xBTree_entry (poNode, mplsTunnelPerfEntry_t, oBTreeNode);
}
	...
	uint32_t u32EgressLSRId)
{
	...
	poTmpEntry->u32IngressLSRId = u32IngressLSRId;
	poTmpEntry->u32EgressLSRId = u32EgressLSRId;
	if ((poNode = xBTree_nodeFindNext (&poTmpEntry->oBTreeNode, &oMplsTunnelPerfTable_BTree)) == NULL)
	{
		xBuffer_free (poTmpEntry);
		return NULL;
	}
...
{
	if (poEntry == NULL ||
		xBTree_nodeFind (&poEntry->oBTreeNode, &oMplsTunnelPerfTable_BTree) == NULL)
	{
		return;    /* Nothing to remove */
	}
	
	xBTree_nodeRemove (&poEntry->oBTreeNode, &oMplsTunnelPerfTable_BTree);
	xBuffer_free (poEntry);   /* XXX - release any other internal resources */
	return;
}
...
mplsTunnelPerfTable_getFirst (
	...
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	*my_loop_context = xBTree_nodeGetFirst (&oMplsTunnelPerfTable_BTree);
...
mplsTunnelPerfTable_getNext (
	...
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	mplsTunnelPerfEntry_t *poEntry = NULL;
	netsnmp_variable_list *idx = put_index_data;
	...
	poEntry = xBTree_entry (*my_loop_context, mplsTunnelPerfEntry_t, oBTreeNode);
	...
	snmp_set_var_typed_integer (idx, ASN_UNSIGNED, poEntry->u32EgressLSRId);
	*my_data_context = (void*) poEntry;
	*my_loop_context = (void*) xBTree_nodeGetNext (&poEntry->oBTreeNode, &oMplsTunnelPerfTable_BTree);
	return put_index_data;
}
	...
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	...
	poEntry = mplsTunnelPerfTable_getByIndex (
		*idx1->val.integer,
		*idx2->val.integer,
		*idx3->val.integer,
		*idx4->val.integer);
...
/** initialize mplsTunnelCRLDPResTable table mapper **/
void
mplsTunnelCRLDPResTable_init (void)
{
	extern oid mplsTunnelCRLDPResTable_oid[];
	netsnmp_handler_registration *reg;
	netsnmp_iterator_info *iinfo;
	netsnmp_table_registration_info *table_info;
	
	reg = netsnmp_create_handler_registration (
		"mplsTunnelCRLDPResTable", &mplsTunnelCRLDPResTable_mapper,
		mplsTunnelCRLDPResTable_oid, OID_LENGTH (mplsTunnelCRLDPResTable_oid),
		HANDLER_CAN_RWRITE
		);
	...
	netsnmp_table_helper_add_indexes (table_info,
		ASN_UNSIGNED /* index: mplsTunnelResourceIndex */,
		0);
	table_info->min_column = MPLSTUNNELCRLDPRESMEANBURSTSIZE;
	table_info->max_column = MPLSTUNNELCRLDPRESSTORAGETYPE;
	
	iinfo = xBuffer_cAlloc (sizeof (netsnmp_iterator_info));
	iinfo->get_first_data_point = &mplsTunnelCRLDPResTable_getFirst;
	iinfo->get_next_data_point = &mplsTunnelCRLDPResTable_getNext;
	iinfo->get_data_point = &mplsTunnelCRLDPResTable_get;
	iinfo->table_reginfo = table_info;
	iinfo->flags |= NETSNMP_ITERATOR_FLAG_SORTED;
...
static int8_t
mplsTunnelCRLDPResTable_BTreeNodeCmp (
	xBTree_Node_t *pNode1, xBTree_Node_t *pNode2, xBTree_t *pBTree)
{
	register mplsTunnelCRLDPResEntry_t *pEntry1 = xBTree_entry (pNode1, mplsTunnelCRLDPResEntry_t, oBTreeNode);
	register mplsTunnelCRLDPResEntry_t *pEntry2 = xBTree_entry (pNode2, mplsTunnelCRLDPResEntry_t, oBTreeNode);
	
	return
		(pEntry1->u32ResourceIndex < pEntry2->u32ResourceIndex) ? -1:
		(pEntry1->u32ResourceIndex == pEntry2->u32ResourceIndex) ? 0: 1;
}

xBTree_t oMplsTunnelCRLDPResTable_BTree = xBTree_initInline (&mplsTunnelCRLDPResTable_BTreeNodeCmp);

/* create a new row in the table */
mplsTunnelCRLDPResEntry_t *
mplsTunnelCRLDPResTable_createEntry (
	uint32_t u32ResourceIndex)
{
	register mplsTunnelCRLDPResEntry_t *poEntry = NULL;
	...
	poEntry->u32ResourceIndex = u32ResourceIndex;
	if (xBTree_nodeFind (&poEntry->oBTreeNode, &oMplsTunnelCRLDPResTable_BTree) != NULL)
	{
		xBuffer_free (poEntry);
		return NULL;
	}
	...
	poEntry->u32Flags = 0;
	poEntry->u8RowStatus = xRowStatus_notInService_c;
	poEntry->u8StorageType = mplsTunnelCRLDPResStorageType_volatile_c;
	
	xBTree_nodeAdd (&poEntry->oBTreeNode, &oMplsTunnelCRLDPResTable_BTree);
	return poEntry;
}

mplsTunnelCRLDPResEntry_t *
mplsTunnelCRLDPResTable_getByIndex (
	uint32_t u32ResourceIndex)
{
	register mplsTunnelCRLDPResEntry_t *poTmpEntry = NULL;
	register xBTree_Node_t *poNode = NULL;
	...
	poTmpEntry->u32ResourceIndex = u32ResourceIndex;
	if ((poNode = xBTree_nodeFind (&poTmpEntry->oBTreeNode, &oMplsTunnelCRLDPResTable_BTree)) == NULL)
	{
		xBuffer_free (poTmpEntry);
		return NULL;
	}
	...
	xBuffer_free (poTmpEntry);
	return xBTree_entry (poNode, mplsTunnelCRLDPResEntry_t, oBTreeNode);
}

mplsTunnelCRLDPResEntry_t *
mplsTunnelCRLDPResTable_getNextIndex (
	uint32_t u32ResourceIndex)
{
	register mplsTunnelCRLDPResEntry_t *poTmpEntry = NULL;
	register xBTree_Node_t *poNode = NULL;
	...
	poTmpEntry->u32ResourceIndex = u32ResourceIndex;
	if ((poNode = xBTree_nodeFindNext (&poTmpEntry->oBTreeNode, &oMplsTunnelCRLDPResTable_BTree)) == NULL)
	{
		xBuffer_free (poTmpEntry);
		return NULL;
	}
...
/* remove a row from the table */
void
mplsTunnelCRLDPResTable_removeEntry (mplsTunnelCRLDPResEntry_t *poEntry)
{
	if (poEntry == NULL ||
		xBTree_nodeFind (&poEntry->oBTreeNode, &oMplsTunnelCRLDPResTable_BTree) == NULL)
	{
		return;    /* Nothing to remove */
	}
	
	xBTree_nodeRemove (&poEntry->oBTreeNode, &oMplsTunnelCRLDPResTable_BTree);
	xBuffer_free (poEntry);   /* XXX - release any other internal resources */
	return;
}
...
/* example iterator hook routines - using 'getNext' to do most of the work */
netsnmp_variable_list *
mplsTunnelCRLDPResTable_getFirst (
	void **my_loop_context, void **my_data_context,
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	*my_loop_context = xBTree_nodeGetFirst (&oMplsTunnelCRLDPResTable_BTree);
	return mplsTunnelCRLDPResTable_getNext (my_loop_context, my_data_context, put_index_data, mydata);
}

netsnmp_variable_list *
mplsTunnelCRLDPResTable_getNext (
	void **my_loop_context, void **my_data_context,
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	mplsTunnelCRLDPResEntry_t *poEntry = NULL;
	netsnmp_variable_list *idx = put_index_data;
	...
	poEntry = xBTree_entry (*my_loop_context, mplsTunnelCRLDPResEntry_t, oBTreeNode);
	
	snmp_set_var_typed_integer (idx, ASN_UNSIGNED, poEntry->u32ResourceIndex);
	*my_data_context = (void*) poEntry;
	*my_loop_context = (void*) xBTree_nodeGetNext (&poEntry->oBTreeNode, &oMplsTunnelCRLDPResTable_BTree);
	return put_index_data;
}

bool
mplsTunnelCRLDPResTable_get (
	void **my_data_context,
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	mplsTunnelCRLDPResEntry_t *poEntry = NULL;
	...
	poEntry = mplsTunnelCRLDPResTable_getByIndex (
		*idx1->val.integer);
	if (poEntry == NULL)
	{
		return false;
...
int
mplsTunnelCRLDPResTable_mapper (
	netsnmp_mib_handler *handler,
	netsnmp_handler_registration *reginfo,
	netsnmp_agent_request_info *reqinfo,
	netsnmp_request_info *requests)
{
	netsnmp_request_info *request;
	netsnmp_table_request_info *table_info;
	mplsTunnelCRLDPResEntry_t *table_entry;
	void *pvOldDdata = NULL;
	int ret;
	...
	case MODE_GET:
		for (request = requests; request != NULL; request = request->next)
		{
			table_entry = (mplsTunnelCRLDPResEntry_t*) netsnmp_extract_iterator_context (request);
			table_info = netsnmp_extract_table_info (request);
			if (table_entry == NULL)
			{
				netsnmp_set_request_error (reqinfo, request, SNMP_NOSUCHINSTANCE);
			...
			switch (table_info->colnum)
			{
			case MPLSTUNNELCRLDPRESMEANBURSTSIZE:
				snmp_set_var_typed_integer (request->requestvb, ASN_UNSIGNED, table_entry->u32MeanBurstSize);
				break;
			case MPLSTUNNELCRLDPRESEXBURSTSIZE:
				snmp_set_var_typed_integer (request->requestvb, ASN_UNSIGNED, table_entry->u32ExBurstSize);
				break;
			case MPLSTUNNELCRLDPRESFREQUENCY:
				snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->i32Frequency);
				break;
			case MPLSTUNNELCRLDPRESWEIGHT:
				snmp_set_var_typed_integer (request->requestvb, ASN_UNSIGNED, table_entry->u32Weight);
				break;
			case MPLSTUNNELCRLDPRESFLAGS:
				snmp_set_var_typed_integer (request->requestvb, ASN_UNSIGNED, table_entry->u32Flags);
				break;
			case MPLSTUNNELCRLDPRESROWSTATUS:
				snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->u8RowStatus);
				break;
			case MPLSTUNNELCRLDPRESSTORAGETYPE:
				snmp_set_var_typed_integer (request->requestvb, ASN_INTEGER, table_entry->u8StorageType);
				break;
		...
		for (request = requests; request != NULL; request = request->next)
		{
			...
			case MPLSTUNNELCRLDPRESROWSTATUS:
				...
				case RS_CREATEANDWAIT:
					...
					table_entry = mplsTunnelCRLDPResTable_createEntry (
						*idx1->val.integer);
					if (table_entry != NULL)
					{
						netsnmp_insert_iterator_context (request, table_entry);
	...
	case MODE_SET_FREE:
		for (request = requests; request != NULL; request = request->next)
		{
			...
			case MPLSTUNNELCRLDPRESROWSTATUS:
				...
				case RS_CREATEANDGO:
				case RS_CREATEANDWAIT:
					mplsTunnelCRLDPResTable_removeEntry (table_entry);
					netsnmp_request_remove_list_entry (request, ROLLBACK_BUFFER);
					break;
				}
			}
	...
	case MODE_SET_ACTION:
		for (request = requests; request != NULL; request = request->next)
		{
			...
			case MPLSTUNNELCRLDPRESWEIGHT:
				...
				break;
			case MPLSTUNNELCRLDPRESFLAGS:
				if (pvOldDdata == NULL && (pvOldDdata = xBuffer_cAlloc (sizeof (table_entry->u32Flags))) == NULL)
				{
					netsnmp_set_request_error (reqinfo, request, SNMP_ERR_RESOURCEUNAVAILABLE);
					return SNMP_ERR_NOERROR;
				}
				...
				{
					memcpy (pvOldDdata, &table_entry->u32Flags, sizeof (table_entry->u32Flags));
					netsnmp_request_add_list_data (request, netsnmp_create_data_list (ROLLBACK_BUFFER, pvOldDdata, &xBuffer_free));
				}
				
				table_entry->u32Flags = *request->requestvb->val.integer;
				break;
			case MPLSTUNNELCRLDPRESSTORAGETYPE:
				if (pvOldDdata == NULL && (pvOldDdata = xBuffer_cAlloc (sizeof (table_entry->u8StorageType))) == NULL)
				{
		...
		for (request = requests; request != NULL; request = request->next)
		{
			...
			case MPLSTUNNELCRLDPRESROWSTATUS:
				...
				case RS_ACTIVE:
				case RS_CREATEANDGO:
					if (/* TODO : int mplsTunnelCRLDPResTable_dep (...) */ TOBE_REPLACED != TOBE_REPLACED)
					{
						netsnmp_set_request_error (reqinfo, request, SNMP_ERR_INCONSISTENTVALUE);
						return SNMP_ERR_NOERROR;
					}
	...
	case MODE_SET_UNDO:
		for (request = requests; request != NULL; request = request->next)
		{
			...
			case MPLSTUNNELCRLDPRESWEIGHT:
				...
				break;
			case MPLSTUNNELCRLDPRESFLAGS:
				memcpy (&table_entry->u32Flags, pvOldDdata, sizeof (table_entry->u32Flags));
				break;
			case MPLSTUNNELCRLDPRESROWSTATUS:
				switch (*request->requestvb->val.integer)
				{
