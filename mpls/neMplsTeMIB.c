...
static oid neMplsCallLinkDescriptorTable_oid[] = {1,3,6,1,4,1,36969,64,1,15};
...
neMplsTeMIB_init (void)
{
	...
	neMplsCallLinkDescriptorTable_init ();
...
static int8_t
neMplsTunnelTable_BTreeNodeCmp (
	xBTree_Node_t *pNode1, xBTree_Node_t *pNode2, xBTree_t *pBTree)
{
	register neMplsTunnelEntry_t *pEntry1 = xBTree_entry (pNode1, neMplsTunnelEntry_t, oBTreeNode);
	register neMplsTunnelEntry_t *pEntry2 = xBTree_entry (pNode2, neMplsTunnelEntry_t, oBTreeNode);
...
xBTree_t oNeMplsTunnelTable_BTree = xBTree_initInline (&neMplsTunnelTable_BTreeNodeCmp);
	...
	uint32_t u32EgressLSRId)
{
	...
	if (xBTree_nodeFind (&poEntry->oBTreeNode, &oNeMplsTunnelTable_BTree) != NULL)
	{
	...
	xBTree_nodeAdd (&poEntry->oBTreeNode, &oNeMplsTunnelTable_BTree);
	return poEntry;
}
...
neMplsTunnelTable_getByIndex (
	...
	uint32_t u32EgressLSRId)
{
	register neMplsTunnelEntry_t *poTmpEntry = NULL;
	...
	if ((poNode = xBTree_nodeFind (&poTmpEntry->oBTreeNode, &oNeMplsTunnelTable_BTree)) == NULL)
	{
	...
	xBuffer_free (poTmpEntry);
	return xBTree_entry (poNode, neMplsTunnelEntry_t, oBTreeNode);
}
	...
	uint32_t u32EgressLSRId)
{
	...
	poTmpEntry->u32IngressLSRId = u32IngressLSRId;
	poTmpEntry->u32EgressLSRId = u32EgressLSRId;
	if ((poNode = xBTree_nodeFindNext (&poTmpEntry->oBTreeNode, &oNeMplsTunnelTable_BTree)) == NULL)
	{
		xBuffer_free (poTmpEntry);
		return NULL;
	}
...
{
	if (poEntry == NULL ||
		xBTree_nodeFind (&poEntry->oBTreeNode, &oNeMplsTunnelTable_BTree) == NULL)
	{
		return;    /* Nothing to remove */
	}
	
	xBTree_nodeRemove (&poEntry->oBTreeNode, &oNeMplsTunnelTable_BTree);
...
{
	*my_loop_context = xBTree_nodeGetFirst (&oNeMplsTunnelTable_BTree);
...
neMplsTunnelTable_getNext (
	...
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	neMplsTunnelEntry_t *poEntry = NULL;
	...
	poEntry = xBTree_entry (*my_loop_context, neMplsTunnelEntry_t, oBTreeNode);
	...
	*my_loop_context = (void*) xBTree_nodeGetNext (&poEntry->oBTreeNode, &oNeMplsTunnelTable_BTree);
	...
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	...
	poEntry = neMplsTunnelTable_getByIndex (
		*idx1->val.integer,
		*idx2->val.integer,
		*idx3->val.integer,
		*idx4->val.integer);
...
static int8_t
neMplsTunnelHopTable_BTreeNodeCmp (
	xBTree_Node_t *pNode1, xBTree_Node_t *pNode2, xBTree_t *pBTree)
{
	register neMplsTunnelHopEntry_t *pEntry1 = xBTree_entry (pNode1, neMplsTunnelHopEntry_t, oBTreeNode);
	register neMplsTunnelHopEntry_t *pEntry2 = xBTree_entry (pNode2, neMplsTunnelHopEntry_t, oBTreeNode);
...
xBTree_t oNeMplsTunnelHopTable_BTree = xBTree_initInline (&neMplsTunnelHopTable_BTreeNodeCmp);
	...
	uint32_t u32Index)
{
	...
	poEntry->u32Index = u32Index;
	if (xBTree_nodeFind (&poEntry->oBTreeNode, &oNeMplsTunnelHopTable_BTree) != NULL)
	{
		xBuffer_free (poEntry);
		return NULL;
	}
	
	xBTree_nodeAdd (&poEntry->oBTreeNode, &oNeMplsTunnelHopTable_BTree);
	return poEntry;
}
...
{
	if (poEntry == NULL ||
		xBTree_nodeFind (&poEntry->oBTreeNode, &oNeMplsTunnelHopTable_BTree) == NULL)
	{
		return;    /* Nothing to remove */
	}
	
	xBTree_nodeRemove (&poEntry->oBTreeNode, &oNeMplsTunnelHopTable_BTree);
	xBuffer_free (poEntry);   /* XXX - release any other internal resources */
	return;
}
...
neMplsTunnelHopTable_getFirst (
	...
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	*my_loop_context = xBTree_nodeGetFirst (&oNeMplsTunnelHopTable_BTree);
...
neMplsTunnelHopTable_getNext (
	...
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	neMplsTunnelHopEntry_t *poEntry = NULL;
	netsnmp_variable_list *idx = put_index_data;
	...
	poEntry = xBTree_entry (*my_loop_context, neMplsTunnelHopEntry_t, oBTreeNode);
	...
	snmp_set_var_typed_integer (idx, ASN_UNSIGNED, poEntry->u32Index);
	*my_data_context = (void*) poEntry;
	*my_loop_context = (void*) xBTree_nodeGetNext (&poEntry->oBTreeNode, &oNeMplsTunnelHopTable_BTree);
	return put_index_data;
}
	...
	netsnmp_request_info *requests)
{
	...
	case MODE_GET:
		for (request = requests; request != NULL; request = request->next)
		{
			table_entry = (neMplsTunnelHopEntry_t*) netsnmp_extract_iterator_context (request);
			table_info = netsnmp_extract_table_info (request);
			if (table_entry == NULL)
			{
				netsnmp_set_request_error (reqinfo, request, SNMP_NOSUCHINSTANCE);
		...
		for (request = requests; request != NULL; request = request->next)
		{
			...
			case NEMPLSTUNNELHOPREVERSELABEL:
				if (table_entry == NULL)
				{
					...
					table_entry = neMplsTunnelHopTable_createEntry (
						*idx1->val.integer,
						*idx2->val.integer,
						*idx3->val.integer);
					if (table_entry != NULL)
...
/** initialize neMplsCallLinkDescriptorTable table mapper **/
void
neMplsCallLinkDescriptorTable_init (void)
{
	extern oid neMplsCallLinkDescriptorTable_oid[];
	netsnmp_handler_registration *reg;
	netsnmp_iterator_info *iinfo;
	netsnmp_table_registration_info *table_info;
	
	reg = netsnmp_create_handler_registration (
		"neMplsCallLinkDescriptorTable", &neMplsCallLinkDescriptorTable_mapper,
		neMplsCallLinkDescriptorTable_oid, OID_LENGTH (neMplsCallLinkDescriptorTable_oid),
		HANDLER_CAN_RONLY
		);
	...
	netsnmp_table_helper_add_indexes (table_info,
		...
		ASN_UNSIGNED /* index: neMplsCallLinkIndex */,
		0);
	table_info->min_column = NEMPLSCALLLINKDESCRIPTORSWITCHINGCAPABILITY;
	table_info->max_column = NEMPLSCALLLINKDESCRIPTORINDICATION;
	
	iinfo = xBuffer_cAlloc (sizeof (netsnmp_iterator_info));
	iinfo->get_first_data_point = &neMplsCallLinkDescriptorTable_getFirst;
	iinfo->get_next_data_point = &neMplsCallLinkDescriptorTable_getNext;
	iinfo->get_data_point = &neMplsCallLinkDescriptorTable_get;
	iinfo->table_reginfo = table_info;
	iinfo->flags |= NETSNMP_ITERATOR_FLAG_SORTED;
...
static int8_t
neMplsCallLinkDescriptorTable_BTreeNodeCmp (
	xBTree_Node_t *pNode1, xBTree_Node_t *pNode2, xBTree_t *pBTree)
{
	register neMplsCallLinkDescriptorEntry_t *pEntry1 = xBTree_entry (pNode1, neMplsCallLinkDescriptorEntry_t, oBTreeNode);
	register neMplsCallLinkDescriptorEntry_t *pEntry2 = xBTree_entry (pNode2, neMplsCallLinkDescriptorEntry_t, oBTreeNode);
	
	return
		(pEntry1->u32IngressLsrId < pEntry2->u32IngressLsrId) ||
		(pEntry1->u32IngressLsrId == pEntry2->u32IngressLsrId && pEntry1->u32EgressLsrId < pEntry2->u32EgressLsrId) ||
		(pEntry1->u32IngressLsrId == pEntry2->u32IngressLsrId && pEntry1->u32EgressLsrId == pEntry2->u32EgressLsrId && pEntry1->u32Id < pEntry2->u32Id) ||
		(pEntry1->u32IngressLsrId == pEntry2->u32IngressLsrId && pEntry1->u32EgressLsrId == pEntry2->u32EgressLsrId && pEntry1->u32Id == pEntry2->u32Id && pEntry1->u32LinkIndex < pEntry2->u32LinkIndex) ? -1:
		(pEntry1->u32IngressLsrId == pEntry2->u32IngressLsrId && pEntry1->u32EgressLsrId == pEntry2->u32EgressLsrId && pEntry1->u32Id == pEntry2->u32Id && pEntry1->u32LinkIndex == pEntry2->u32LinkIndex) ? 0: 1;
}

xBTree_t oNeMplsCallLinkDescriptorTable_BTree = xBTree_initInline (&neMplsCallLinkDescriptorTable_BTreeNodeCmp);

/* create a new row in the table */
neMplsCallLinkDescriptorEntry_t *
neMplsCallLinkDescriptorTable_createEntry (
	uint32_t u32IngressLsrId,
	uint32_t u32EgressLsrId,
	uint32_t u32Id,
	uint32_t u32LinkIndex)
{
	register neMplsCallLinkDescriptorEntry_t *poEntry = NULL;
	...
	if (xBTree_nodeFind (&poEntry->oBTreeNode, &oNeMplsCallLinkDescriptorTable_BTree) != NULL)
	{
		xBuffer_free (poEntry);
		return NULL;
	}
	
	xBTree_nodeAdd (&poEntry->oBTreeNode, &oNeMplsCallLinkDescriptorTable_BTree);
	return poEntry;
}

neMplsCallLinkDescriptorEntry_t *
neMplsCallLinkDescriptorTable_getByIndex (
	uint32_t u32IngressLsrId,
	uint32_t u32EgressLsrId,
	uint32_t u32Id,
	uint32_t u32LinkIndex)
{
	register neMplsCallLinkDescriptorEntry_t *poTmpEntry = NULL;
	register xBTree_Node_t *poNode = NULL;
	...
	if ((poNode = xBTree_nodeFind (&poTmpEntry->oBTreeNode, &oNeMplsCallLinkDescriptorTable_BTree)) == NULL)
	{
		xBuffer_free (poTmpEntry);
		return NULL;
	}
	...
	xBuffer_free (poTmpEntry);
	return xBTree_entry (poNode, neMplsCallLinkDescriptorEntry_t, oBTreeNode);
}

neMplsCallLinkDescriptorEntry_t *
neMplsCallLinkDescriptorTable_getNextIndex (
	uint32_t u32IngressLsrId,
	uint32_t u32EgressLsrId,
	uint32_t u32Id,
	uint32_t u32LinkIndex)
{
	...
	poTmpEntry->u32Id = u32Id;
	poTmpEntry->u32LinkIndex = u32LinkIndex;
	if ((poNode = xBTree_nodeFindNext (&poTmpEntry->oBTreeNode, &oNeMplsCallLinkDescriptorTable_BTree)) == NULL)
	{
		xBuffer_free (poTmpEntry);
		return NULL;
	}
...
/* remove a row from the table */
void
neMplsCallLinkDescriptorTable_removeEntry (neMplsCallLinkDescriptorEntry_t *poEntry)
{
	if (poEntry == NULL ||
		xBTree_nodeFind (&poEntry->oBTreeNode, &oNeMplsCallLinkDescriptorTable_BTree) == NULL)
	{
		return;    /* Nothing to remove */
	}
	
	xBTree_nodeRemove (&poEntry->oBTreeNode, &oNeMplsCallLinkDescriptorTable_BTree);
	xBuffer_free (poEntry);   /* XXX - release any other internal resources */
	return;
}
...
/* example iterator hook routines - using 'getNext' to do most of the work */
netsnmp_variable_list *
neMplsCallLinkDescriptorTable_getFirst (
	void **my_loop_context, void **my_data_context,
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	*my_loop_context = xBTree_nodeGetFirst (&oNeMplsCallLinkDescriptorTable_BTree);
	return neMplsCallLinkDescriptorTable_getNext (my_loop_context, my_data_context, put_index_data, mydata);
}

netsnmp_variable_list *
neMplsCallLinkDescriptorTable_getNext (
	void **my_loop_context, void **my_data_context,
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	neMplsCallLinkDescriptorEntry_t *poEntry = NULL;
	netsnmp_variable_list *idx = put_index_data;
	...
	poEntry = xBTree_entry (*my_loop_context, neMplsCallLinkDescriptorEntry_t, oBTreeNode);
	...
	*my_data_context = (void*) poEntry;
	*my_loop_context = (void*) xBTree_nodeGetNext (&poEntry->oBTreeNode, &oNeMplsCallLinkDescriptorTable_BTree);
	return put_index_data;
}

bool
neMplsCallLinkDescriptorTable_get (
	void **my_data_context,
	netsnmp_variable_list *put_index_data, netsnmp_iterator_info *mydata)
{
	neMplsCallLinkDescriptorEntry_t *poEntry = NULL;
	...
	poEntry = neMplsCallLinkDescriptorTable_getByIndex (
		*idx1->val.integer,
		*idx2->val.integer,
		*idx3->val.integer,
		*idx4->val.integer);
...
/* neMplsCallLinkDescriptorTable table mapper */
int
neMplsCallLinkDescriptorTable_mapper (
	netsnmp_mib_handler *handler,
	netsnmp_handler_registration *reginfo,
	netsnmp_agent_request_info *reqinfo,
	netsnmp_request_info *requests)
{
	netsnmp_request_info *request;
	netsnmp_table_request_info *table_info;
	neMplsCallLinkDescriptorEntry_t *table_entry;
	...
	case MODE_GET:
		for (request = requests; request != NULL; request = request->next)
		{
			table_entry = (neMplsCallLinkDescriptorEntry_t*) netsnmp_extract_iterator_context (request);
			table_info = netsnmp_extract_table_info (request);
			if (table_entry == NULL)
			{
				netsnmp_set_request_error (reqinfo, request, SNMP_NOSUCHINSTANCE);
			...
			case NEMPLSCALLLINKDESCRIPTORMINLSPBANDWIDTH:
				...
				break;
			case NEMPLSCALLLINKDESCRIPTORMAXLSPBANDWIDTHPRIO0:
				...
				break;
			case NEMPLSCALLLINKDESCRIPTORMAXLSPBANDWIDTHPRIO1:
				...
				break;
			case NEMPLSCALLLINKDESCRIPTORMAXLSPBANDWIDTHPRIO2:
				...
				break;
			case NEMPLSCALLLINKDESCRIPTORMAXLSPBANDWIDTHPRIO3:
				...
				break;
			case NEMPLSCALLLINKDESCRIPTORMAXLSPBANDWIDTHPRIO4:
				...
				break;
			case NEMPLSCALLLINKDESCRIPTORMAXLSPBANDWIDTHPRIO5:
				...
				break;
			case NEMPLSCALLLINKDESCRIPTORMAXLSPBANDWIDTHPRIO6:
				...
				break;
			case NEMPLSCALLLINKDESCRIPTORMAXLSPBANDWIDTHPRIO7:
